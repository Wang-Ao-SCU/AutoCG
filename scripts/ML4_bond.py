...
class BondParameterDataset:
    ...
    def __init__(self, data_path):
        ...
        self.valid_indices = self._filter_valid_smiles_pairs()

    def _load_data(self, path):
        data = pd.read_csv(path)
        required_columns = ['smiles_1', 'smiles_2', 'r', 'k']
        if not all((col in data.columns for col in required_columns)):
            raise ValueError(f'The CSV file must contain the following columns: {required_columns}')
        data = data.dropna(subset=['smiles_1', 'smiles_2', 'r', 'k'])
        return data

    def _filter_valid_smiles_pairs(self):
        valid_indices = []
    ...
    def get_processed_data(self):
        ...
        return (X_smiles_a, X_smiles_b, y_r, y_k)
...
def get_pair_enhanced_features(smiles_a_list, smiles_b_list):
    ...
    return (pair_features, desc_names)
...
class ModelPipeline:
    ...
    def __init__(self, data_path, save_path='./bond_results'):
        self.dataset = BondParameterDataset(data_path)
        self.X_smiles_a, self.X_smiles_b, self.y_r, self.y_k = self.dataset.get_processed_data()
        self.X_features, _ = get_pair_enhanced_features(self.X_smiles_a, self.X_smiles_b)
    ...
    def predict_single_pair(self, smiles_a, smiles_b, model_type):
        ...
        X = get_pair_enhanced_features([smiles_a], [smiles_b])[0]
        X = np.nan_to_num(X)
        ...
        return {'model_type': model_type.upper(), 'smiles_a': smiles_a, 'smiles_b': smiles_b, 'predicted_r': round(y_pred_r, 4), 'predicted_k': round(y_pred_k, 4)}
...
def main():
    DATA_PATH = './test/bond_data.csv'
    SAVE_PATH = './bond_prediction_results'
    TEST_SIZE = 0.2
    ...
    try:
        ...
        for model_type in TRAIN_MODEL_TYPES:
            if model_type in pipeline.trained_models:
                pred_result = pipeline.predict_single_pair(test_smiles_a, test_smiles_b, model_type)
        ...
        for model_type in TRAIN_MODEL_TYPES:
            if model_type in new_pipeline.trained_models:
                pred_result = new_pipeline.predict_single_pair(test_smiles_a, test_smiles_b, model_type)
    except Exception as e:
        import traceback
        traceback.print_exc()

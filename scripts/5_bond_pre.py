...
def get_pair_enhanced_features(smiles_a_list, smiles_b_list):
    desc_names = [desc[0] for desc in Descriptors._descList]
    features_a = []
    features_b = []
    for smiles_a, smiles_b in zip(smiles_a_list, smiles_b_list):
    ...
    features_diff = np.abs(features_a - features_b)
    pair_features = np.hstack([features_a, features_b, features_diff])
...
class BondParameterPredictor:
    ...
    def load_models(self, model_types=['ridge', 'gbr']):
        ...
        if self.loaded_models:
            ...
            raise RuntimeError('No valid models were loaded. Check the model directory and files')

    def predict_batch(self, input_df):
        required_cols = ['smiles1', 'smiles2']
        if not all((col in input_df.columns for col in required_cols)):
            raise ValueError(f'The input CSV must contain columns: {required_cols}')
        valid_mask = []
        for _, row in input_df.iterrows():
            smiles1, smiles2 = (row['smiles1'], row['smiles2'])
            mol1 = Chem.MolFromSmiles(smiles1)
            mol2 = Chem.MolFromSmiles(smiles2)
            valid = mol1 is not None and mol2 is not None
            valid_mask.append(valid)
            if not valid:
                pass
        valid_df = input_df[valid_mask].copy()
        if len(valid_df) == 0:
            raise ValueError('No valid SMILES pairs were found in the input data; prediction cannot proceed')
        X_features = get_pair_enhanced_features(smiles_a_list=valid_df['smiles1'].tolist(), smiles_b_list=valid_df['smiles2'].tolist())
        for model_type, models in self.loaded_models.items():
            pred_r = models['r'].predict(X_features)
            pred_k = models['k'].predict(X_features)
            valid_df[f'{model_type}_r_pred'] = np.round(pred_r, 6)
            valid_df[f'{model_type}_k_pred'] = np.round(pred_k, 6)
        result_df = input_df.copy()
        for model_type in self.loaded_models.keys():
            result_df[f'{model_type}_r_pred'] = np.nan
            result_df[f'{model_type}_k_pred'] = np.nan
        result_df.loc[valid_mask, [f'{model_type}_r_pred' for model_type in self.loaded_models.keys()]] = valid_df[[f'{model_type}_r_pred' for model_type in self.loaded_models.keys()]].values
        result_df.loc[valid_mask, [f'{model_type}_k_pred' for model_type in self.loaded_models.keys()]] = valid_df[[f'{model_type}_k_pred' for model_type in self.loaded_models.keys()]].values
        return result_df

def main():
    INPUT_CSV_PATH = './output/bonds.csv'
    OUTPUT_CSV_PATH = './all_bondtypes_with_predictions.csv'
    MODEL_DIR = './ML4/bond_prediction_results/models'
    MODEL_TYPES = ['ridge', 'gbr']
    try:
        input_df = pd.read_csv(INPUT_CSV_PATH)
        predictor = BondParameterPredictor(model_dir=MODEL_DIR)
